...
const DungeonSystem = {
    ...
    dungeons: [
        {
            ...
            difficulty: 1,
            backgroundImage: 'img/mainmap12.png', // Customizable background for beginner dungeon
        ...
        },
        {
            ...
            monsterTypes: ['goblin', 'skeleton'],
            recommendedLevel: 5,
            rewards: {
                copperMultiplier: 1.5,
        ...
        },
        {
            ...
            monsterTypes: ['orc', 'dragon'],
            recommendedLevel: 15,
            rewards: {
                copperMultiplier: 2.5,
    ...
    init: function(savedData = null) {
        ...
        if (savedData && savedData.currentDungeonId) {
            const savedDungeon = this.getDungeonById(savedData.currentDungeonId);
            if (savedDungeon) {
                this.currentDungeon = savedDungeon;
            }
        }
    ...
    selectDungeon: function(dungeonId) {
        ...
        // IMPORTANT: Reset the distance when entering a new dungeon
        // This completely resets the map progress
        ...
        // Set allowed monster types for this dungeon
        MapSystem.allowedMonsterTypes = dungeon.monsterTypes;
        
        // Ensure a high monster spawn rate regardless of dungeon difficulty
        // But still make harder dungeons spawn slightly more monsters
        MapSystem.monsterSpawnRate = 0.08 + (dungeon.difficulty * 0.01); // 0.09 to 0.13
        
        // Clear existing monsters
        MonsterSystem.clearAllMonsters();
        
        // Update rewards
        this.updateRewardMultipliers();
        
        // Play dungeon-specific BGM if AudioSystem is available
        if (typeof AudioSystem !== 'undefined') {
            AudioSystem.playBackgroundMusic(dungeon.id);
        }
        ...
        Utils.showNotification(
            ...
            `Difficulty: ${dungeon.difficulty} | Recommended Level: ${dungeon.recommendedLevel}`,
            5000
        );
        
        // Auto-deploy all available characters to the dungeon
        if (typeof CharacterSystem !== 'undefined') {
            console.log("Auto-deploying all characters to dungeon");
            CharacterSystem.characters.forEach(character => {
                if (!CharacterSystem.isCharacterActive(character.id)) {

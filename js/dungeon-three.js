...
const DungeonThreeEnvironment = {
    ...
    validateImagePath: function(imagePath, dungeonId) {
        ...
        const backupImages = {
            'cyber_slums': 'img/neon_district.png',
    ...
    selectDungeon: function(dungeonId) {
        if (!dungeonId || !this.dungeonModels[dungeonId]) return;
        
        // Deactivate all portal effects first
        this.deactivateAllPortalEffects();
        
        // Update selection state for all dungeons
        Object.keys(this.dungeonModels).forEach(id => {
            const model = this.dungeonModels[id];
            const isSelected = (id === dungeonId);
            ...
            if (isSelected) {
                // Selected effects
                model.platform.material.emissiveIntensity = 0.5;
                model.glowRing.scale.set(1.1, 1.1, 1.1);
                
                // Activate portal effect
                this.activatePortalEffect(dungeonId);
                
                // Create floating holographic information about the dungeon
                // this.showDungeonInfo(dungeonId, model.portal.position);
            } else {
                // Deselected effects
                model.platform.material.emissiveIntensity = 0.2;
                model.glowRing.scale.set(1.0, 1.0, 1.0);
            }
        });
        
        // Use DungeonSystem to actually select the dungeon
        DungeonSystem.selectDungeon(dungeonId);

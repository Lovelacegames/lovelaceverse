...
const Inventory = {
    ...
    init: function(savedData = null) {
        // Initialize empty inventory - we'll use a larger array to accommodate multiple tabs
    ...
    equipItem: function(slotIndex, characterId = null) {
        ...
        if (secondarySlot === 'leftHand') {
            // We don't put the actual item in the secondary slot
            // Just mark it as used by setting a reference to the primary
    ...
    addItem: function(itemId, quantity = 1) {
        ...
        if (item.stackable) {
            ...
            for (let i = 0; i < this.slots.length; i++) {
                ...
                if (slot && slot.id === itemId && slot.quantity < slot.maxStack) {
        ...
        // If we get here, we need to add to a new slot
        // (either the item is not stackable, or we couldn't fit all in existing stacks)
        while (quantity > 0) {
            // Find empty slot
            const emptySlotIndex = this.slots.findIndex(slot => slot === null);
            if (emptySlotIndex === -1) {
            ...
            const newItem = Items.createItem(itemId, item.stackable ? Math.min(quantity, item.maxStack) : 1);
        ...
        // Only update UI if inventory modal is currently open
        if (document.getElementById('inventory-modal')) {
            this.updateInventoryUI();
        }
    ...
    getEquipmentStats: function(characterId = null) {
        ...
        // Get equipped items for the character
